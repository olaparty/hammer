...
const mockJsonToPbAction = async (fileName: string, args?: ReadonlyArray<string>, cwd?: string | undefined): Promise<string> => {
    ...
    // Get relative path from workspace root to the folder (not the file)
    const relativePath = path.relative(workspaceRoot, fileDirectory);
    const newArgs = ['mock-convert-pb-folder', `folder=${relativePath}`];
    const env = {};
...
export const genMockJsonToPbInCurrentFolderCommand = (args: any) => {
